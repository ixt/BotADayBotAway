...
while read LINE; do
	USER=$(cut -d/ -f1 <<< "$LINE" )
	VALUE=$(lynx -dump -listonly https://soundcloud.com/$USER/likes | cut -d. -f2- | grep "://soundcloud" | sed -e "s/^ //g;/oembed/d;/.xml/d;/\/$/d;/popular\/searches/d" | cut -d/ -f4- | grep "/" | wc -l )
...
# And post
../../../Tools/tweet.sh/tweet.sh post "$LASTUSER liked $FINALURL #SoundChain"
popd
